...
static pointer F4522coplanar_fe_intersection();
static pointer F4523find_next_segment();
static pointer F4524find_loop();
static pointer F4525punch_hole();
static pointer F4526construct_polygon();
static pointer F4527remove_non_overlapping_border();
static pointer F4528coplanar_ff_intersection();
static pointer F4529face_();
static pointer F4530face_();
static pointer F4531non_coplanar_fe_relation();
static pointer F4532ff_relation();
static pointer F4533bb_relation();
static pointer F4534make_lines();
static pointer F4535copy_face();

/*coplanar-fe-intersection*/
static pointer F4522coplanar_fe_intersection(ctx,n,argv,env)
register context *ctx;
register int n; register pointer argv[]; pointer env;
{ register pointer *local=ctx->vsp, w, *fqv=qv;
  numunion nu;
	if (n<2) maerror();
	if (n>=3) { local[0]=(argv[2]); goto ENT4539;}
	local[0]= fqv[0];
ENT4539:
	if (n>=4) { local[1]=(argv[3]); goto ENT4538;}
...
BLK4543:
	...
	local[11]= NIL;
	local[12]= local[5];
	if (local[0]!=local[12]) goto IF4551;
	local[12]= NIL;
	local[13]= local[2];
	local[14]= local[3];
	ctx->vsp=local+15;
...
IF4550:
	...
	ctx->vsp=local+13;
	local[13]= makeclosure(codevec,quotevec,CLO4553,env,argv,local);
	ctx->vsp=local+14;
...
IF4560:
	...
	local[5] = w;
	local[13]= local[5];
	if (local[0]!=local[13]) goto IF4563;
	local[13]= T;
	local[14]= argv[1];
	local[15]= fqv[8];
	local[16]= local[6];
...
/*find-next-segment*/
static pointer F4523find_next_segment(ctx,n,argv,env)
register context *ctx;
register int n; register pointer argv[]; pointer env;
{ register pointer *local=ctx->vsp, w, *fqv=qv;
  numunion nu;
...
/*find-loop*/
static pointer F4524find_loop(ctx,n,argv,env)
register context *ctx;
register int n; register pointer argv[]; pointer env;
{ register pointer *local=ctx->vsp, w, *fqv=qv;
  numunion nu;
	if (n<2) maerror();
	if (n>=3) { local[0]=(argv[2]); goto ENT4583;}
...
WHL4585:
	...
	local[11]= local[0];
	ctx->vsp=local+12;
	w=(pointer)F4523find_next_segment(ctx,3,local+9); /*find-next-segment*/
	local[6] = w;
	if (local[6]==NIL) goto WHX4586;
	local[9]= local[6];
	local[10]= argv[0];
	local[11]= fqv[12];
...
BLK4587:
	...
	ctx->vsp=local+11;
	w=(*ftab[3])(ctx,2,local+9,&ftab[3],fqv[14]); /*eps-v=*/
	if (w!=NIL) goto CON4591;
	local[9]= NIL;
	local[10]= local[1];
WHL4593:
	if (local[10]==NIL) goto WHX4594;
	w=local[10];
	if (!iscons(w) && w!=NIL) error(E_NOLIST);
...
/*punch-hole*/
static pointer F4525punch_hole(ctx,n,argv,env)
register context *ctx;
register int n; register pointer argv[]; pointer env;
{ register pointer *local=ctx->vsp, w, *fqv=qv;
  numunion nu;
...
/*construct-polygon*/
static pointer F4526construct_polygon(ctx,n,argv,env)
register context *ctx;
register int n; register pointer argv[]; pointer env;
{ register pointer *local=ctx->vsp, w, *fqv=qv;
  numunion nu;
	if (n<2) maerror();
	if (n>=3) { local[0]=(argv[2]); goto ENT4604;}
	local[0]= loadglobal(fqv[10]);
ENT4604:
ENT4603:
	if (n>3) maerror();
	...
	local[6]= NIL;
WHL4605:
	if (argv[0]==NIL) goto WHX4606;
	local[7]= argv[0];
	local[8]= argv[1];
	ctx->vsp=local+9;
	w=(pointer)F4524find_loop(ctx,2,local+7); /*find-loop*/
	local[4] = w;
	local[7]= NIL;
	w=local[4];
	if (!iscons(w) && w!=NIL) error(E_NOLIST);
...
WHL4615:
	w=local[8];
	if (!iscons(w) && w!=NIL) error(E_NOLIST);
	if ((w)->c.cons.cdr==NIL) goto WHX4616;
	w=local[8];
	if (!iscons(w) && w!=NIL) error(E_NOLIST);
	local[9]= (w)->c.cons.car;
	w=local[8];
...
WHL4620:
	...
	if (w==NIL) goto CON4625;
	local[11]= local[7];
	local[12]= local[9];
	ctx->vsp=local+13;
	w=(pointer)F4525punch_hole(ctx,2,local+11); /*punch-hole*/
	local[11]= local[9];
	local[12]= local[1];
	local[13]= fqv[12];
...
static pointer F4527remove_non_overlapping_border(ctx,n,argv,env)
register context *ctx;
register int n; register pointer argv[]; pointer env;
{ register pointer *local=ctx->vsp, w, *fqv=qv;
  numunion nu;
...
static pointer F4528coplanar_ff_intersection(ctx,n,argv,env)
register context *ctx;
register int n; register pointer argv[]; pointer env;
{ register pointer *local=ctx->vsp, w, *fqv=qv;
  numunion nu;
	if (n<2) maerror();
	if (n>=3) { local[0]=(argv[2]); goto ENT4638;}
	local[0]= fqv[0];
ENT4638:
	if (n>=4) { local[1]=(argv[3]); goto ENT4637;}
	local[1]= loadglobal(fqv[1]);
ENT4637:
ENT4636:
	if (n>4) maerror();
...
WHL4640:
	...
	local[13]= local[1];
	ctx->vsp=local+14;
	w=(pointer)F4522coplanar_fe_intersection(ctx,4,local+10); /*coplanar-fe-intersection*/
	local[6] = w;
	w=local[6];
	if (!iscons(w) && w!=NIL) error(E_NOLIST);
	if ((w)->c.cons.car==NIL) goto IF4644;
	w=local[6];
	if (!iscons(w) && w!=NIL) error(E_NOLIST);
	local[10]= (w)->c.cons.car;
	local[11]= local[4];
...
IF4645:
	...
	w=(w)->c.cons.cdr;
	if (!iscons(w) && w!=NIL) error(E_NOLIST);
	if ((w)->c.cons.car==NIL) goto IF4646;
	w=local[6];
	if (!iscons(w) && w!=NIL) error(E_NOLIST);
	w=(w)->c.cons.cdr;
	if (!iscons(w) && w!=NIL) error(E_NOLIST);
...
IF4648:
	local[8]= NIL;
IF4649:
	if (T==NIL) goto IF4650;
	local[8]= argv[1];
	local[9]= argv[0];
	local[10]= local[4];
	ctx->vsp=local+11;
	w=(pointer)F4527remove_non_overlapping_border(ctx,3,local+8); /*remove-non-overlapping-border*/
	local[4] = w;
	local[8]= local[4];
	goto IF4651;
IF4650:
...
WHL4653:
	...
	w=local[6];
	if (!iscons(w) && w!=NIL) error(E_NOLIST);
	if ((w)->c.cons.car==NIL) goto IF4657;
	w=local[6];
	if (!iscons(w) && w!=NIL) error(E_NOLIST);
	local[10]= (w)->c.cons.car;
	local[11]= local[5];
...
IF4658:
	...
	w=(w)->c.cons.cdr;
	if (!iscons(w) && w!=NIL) error(E_NOLIST);
	if ((w)->c.cons.car==NIL) goto IF4659;
	w=local[6];
	if (!iscons(w) && w!=NIL) error(E_NOLIST);
	w=(w)->c.cons.cdr;
	if (!iscons(w) && w!=NIL) error(E_NOLIST);
...
IF4664:
	...
	local[9]= fqv[28];
	ctx->vsp=local+10;
	local[10]= makeclosure(codevec,quotevec,CLO4665,env,argv,local);
	ctx->vsp=local+11;
	w=(*ftab[6])(ctx,3,local+8,&ftab[6],fqv[29]); /*remove-duplicates*/
	local[3] = w;
	if (local[7]!=NIL) goto CON4667;
	ctx->vsp=local+8;
	local[8]= makeclosure(codevec,quotevec,CLO4668,env,argv,local);
	local[9]= local[2];
	ctx->vsp=local+10;
	w=(*ftab[7])(ctx,2,local+8,&ftab[7],fqv[30]); /*every*/
	if (w==NIL) goto CON4667;
	local[8]= argv[0];
	local[9]= fqv[5];
	local[10]= argv[1];
	local[11]= fqv[24];
	...
	if (w==NIL) goto CON4670;
	local[10]= local[8];
	local[11]= fqv[0];
	ctx->vsp=local+12;
	w=(pointer)EQ(ctx,2,local+10); /*eql*/
	if (w==NIL) goto CON4672;
	local[10]= argv[1];
	goto CON4671;
CON4672:
	local[10]= local[9];
...
CON4667:
	...
	local[9]= w;
	ctx->vsp=local+10;
	w=(pointer)F4526construct_polygon(ctx,2,local+8); /*construct-polygon*/
	local[8]= w;
	goto CON4666;
CON4680:
	local[8]= NIL;
...
/*closure or cleaner*/
static pointer CLO4665(ctx,n,argv,env)
register context *ctx;
register int n; register pointer argv[]; pointer env;
{ register pointer *local=ctx->vsp, w, *fqv=qv;
  numunion nu;
...
/*closure or cleaner*/
static pointer CLO4668(ctx,n,argv,env)
register context *ctx;
register int n; register pointer argv[]; pointer env;
{ register pointer *local=ctx->vsp, w, *fqv=qv;
  numunion nu;
...
/*face**/
static pointer F4529face_(ctx,n,argv,env)
register context *ctx;
register int n; register pointer argv[]; pointer env;
{ register pointer *local=ctx->vsp, w, *fqv=qv;
  numunion nu;
	if (n<2) maerror();
	if (n>=3) { local[0]=(argv[2]); goto ENT4686;}
...
ENT4685:
	...
	local[4]= local[0];
	ctx->vsp=local+5;
	w=(pointer)F4528coplanar_ff_intersection(ctx,4,local+1); /*coplanar-ff-intersection*/
	local[0]= w;
BLK4684:
	ctx->vsp=local; return(local[0]);}

/*face+*/
static pointer F4530face_(ctx,n,argv,env)
register context *ctx;
register int n; register pointer argv[]; pointer env;
{ register pointer *local=ctx->vsp, w, *fqv=qv;
  numunion nu;
	if (n<2) maerror();
	if (n>=3) { local[0]=(argv[2]); goto ENT4689;}
	local[0]= loadglobal(fqv[1]);
ENT4689:
ENT4688:
	if (n>3) maerror();
...
static pointer F4531non_coplanar_fe_relation(ctx,n,argv,env)
register context *ctx;
register int n; register pointer argv[]; pointer env;
{ register pointer *local=ctx->vsp, w, *fqv=qv;
  numunion nu;
	if (n<2) maerror();
	if (n>=3) { local[0]=(argv[2]); goto ENT4692;}
	local[0]= loadglobal(fqv[1]);
ENT4692:
ENT4691:
	if (n>3) maerror();
...
CON4694:
	...
	local[4]= local[3];
	local[5]= local[4];
	if (fqv[0]!=local[5]) goto IF4698;
	local[5]= argv[0];
	local[6]= fqv[37];
	local[7]= argv[1]->c.obj.iv[1];
	ctx->vsp=local+8;
	w=(pointer)SEND(ctx,3,local+5); /*send*/
	if (w==NIL) goto IF4700;
	local[5]= argv[0];
	local[6]= fqv[37];
	local[7]= argv[1]->c.obj.iv[1];
	ctx->vsp=local+8;
...
IF4698:
	local[5]= local[4];
	if (fqv[40]!=local[5]) goto IF4702;
	local[5]= fqv[38];
	goto IF4703;
IF4702:
	local[5]= local[4];
	if (fqv[31]!=local[5]) goto IF4704;
	local[5]= NIL;
	goto IF4705;
IF4704:
	local[5]= NIL;
...
/*ff-relation*/
static pointer F4532ff_relation(ctx,n,argv,env)
register context *ctx;
register int n; register pointer argv[]; pointer env;
{ register pointer *local=ctx->vsp, w, *fqv=qv;
  numunion nu;
	if (n<2) maerror();
	if (n>=3) { local[0]=(argv[2]); goto ENT4709;}
	local[0]= loadglobal(fqv[1]);
ENT4709:
ENT4708:
	if (n>3) maerror();
	...
	if (w==NIL) goto CON4711;
	local[13]= local[3];
	local[14]= local[4];
	ctx->vsp=local+15;
	w=(*ftab[8])(ctx,2,local+13,&ftab[8],fqv[34]); /*eps=*/
	if (w==NIL) goto IF4712;
	local[13]= fqv[41];
	local[14]= fqv[42];
	ctx->vsp=local+15;
...
CON4714:
	local[13]= NIL;
CON4710:
	if (local[9]==NIL) goto IF4717;
	local[13]= argv[0];
	local[14]= argv[1];
	local[15]= local[0];
	ctx->vsp=local+16;
	w=(pointer)F4529face_(ctx,3,local+13); /*face**/
	local[10] = w;
	local[13]= local[10];
	local[14]= loadglobal(fqv[11]);
	ctx->vsp=local+15;
	w=(pointer)DERIVEDP(ctx,2,local+13); /*derivedp*/
	if (w==NIL) goto CON4720;
	local[13]= local[9];
	local[14]= local[10];
	ctx->vsp=local+15;
...
CON4720:
	ctx->vsp=local+13;
	local[13]= makeclosure(codevec,quotevec,CLO4722,env,argv,local);
	local[14]= local[10];
	ctx->vsp=local+15;
	w=(*ftab[10])(ctx,2,local+13,&ftab[10],fqv[44]); /*some*/
	if (w==NIL) goto CON4721;
	local[13]= local[9];
	local[14]= local[10];
	ctx->vsp=local+15;
	w=(pointer)APPEND(ctx,2,local+13); /*append*/
...
CON4721:
	ctx->vsp=local+13;
	local[13]= makeclosure(codevec,quotevec,CLO4724,env,argv,local);
	local[14]= local[10];
	ctx->vsp=local+15;
	w=(*ftab[7])(ctx,2,local+13,&ftab[7],fqv[30]); /*every*/
	if (w==NIL) goto CON4723;
	local[13]= fqv[45];
	w=local[9];
	if (!iscons(w) && w!=NIL) error(E_NOLIST);
	w=(w)->c.cons.cdr;
...
WHL4726:
	...
	local[17]= local[0];
	ctx->vsp=local+18;
	w=(pointer)F4531non_coplanar_fe_relation(ctx,3,local+15); /*non-coplanar-fe-relation*/
	local[11] = w;
	local[15]= local[11];
	if (fqv[39]!=local[15]) goto IF4730;
	local[15]= T;
	local[16]= fqv[46];
	local[17]= argv[1];
	local[18]= local[13];
...
WHL4735:
	...
	local[11] = w;
	local[15]= local[11];
	if (fqv[39]!=local[15]) goto IF4739;
	local[15]= T;
	local[16]= fqv[47];
	local[17]= argv[0];
	local[18]= local[13];
...
BLK4737:
	w = NIL;
	ctx->vsp=local+13;
	local[13]= makeclosure(codevec,quotevec,CLO4745,env,argv,local);
	local[14]= local[10];
	ctx->vsp=local+15;
	w=(*ftab[7])(ctx,2,local+13,&ftab[7],fqv[30]); /*every*/
	if (w==NIL) goto CON4744;
	local[13]= fqv[48];
	ctx->vsp=local+14;
...
/*closure or cleaner*/
static pointer CLO4724(ctx,n,argv,env)
register context *ctx;
register int n; register pointer argv[]; pointer env;
{ register pointer *local=ctx->vsp, w, *fqv=qv;
  numunion nu;
...
/*bb-relation*/
static pointer F4533bb_relation(ctx,n,argv,env)
register context *ctx;
register int n; register pointer argv[]; pointer env;
{ register pointer *local=ctx->vsp, w, *fqv=qv;
  numunion nu;
	if (n<2) maerror();
	if (n>=3) { local[0]=(argv[2]); goto ENT4749;}
...
IF4762:
	...
	local[18]= local[0];
	ctx->vsp=local+19;
	w=(pointer)F4532ff_relation(ctx,3,local+16); /*ff-relation*/
	local[7] = w;
	local[16]= local[7];
IF4763:
	if (local[7]!=NIL) goto CON4766;
	local[16]= NIL;
	goto CON4765;
CON4766:
	w=local[7];
	if (!iscons(w) && w!=NIL) error(E_NOLIST);
	local[16]= (w)->c.cons.car;
	if (fqv[48]!=local[16]) goto CON4767;
	local[16]= NIL;
	goto CON4765;
CON4767:
	w=local[7];
	if (!iscons(w) && w!=NIL) error(E_NOLIST);
	local[16]= (w)->c.cons.car;
	if (fqv[41]!=local[16]) goto CON4768;
	w=local[7];
	if (!iscons(w) && w!=NIL) error(E_NOLIST);
	w=(w)->c.cons.cdr;
	if (!iscons(w) && w!=NIL) error(E_NOLIST);
	local[16]= (w)->c.cons.car;
	if (fqv[43]!=local[16]) goto CON4768;
	local[16]= fqv[58];
	local[17]= local[12];
	local[18]= local[14];
	w=local[7];
...
CON4768:
	...
	if (!iscons(w) && w!=NIL) error(E_NOLIST);
	local[16]= (w)->c.cons.car;
	if (fqv[45]!=local[16]) goto CON4769;
	w=local[7];
	if (!iscons(w) && w!=NIL) error(E_NOLIST);
	w=(w)->c.cons.cdr;
	if (!iscons(w) && w!=NIL) error(E_NOLIST);
	local[16]= (w)->c.cons.car;
	if (fqv[42]!=local[16]) goto CON4769;
	local[16]= fqv[45];
	local[17]= local[12];
	local[18]= local[14];
	ctx->vsp=local+19;
...
CON4769:
	...
	if (!iscons(w) && w!=NIL) error(E_NOLIST);
	local[16]= (w)->c.cons.car;
	if (fqv[41]!=local[16]) goto CON4770;
	w=local[7];
	if (!iscons(w) && w!=NIL) error(E_NOLIST);
	w=(w)->c.cons.cdr;
	if (!iscons(w) && w!=NIL) error(E_NOLIST);
	local[16]= (w)->c.cons.car;
	if (fqv[42]!=local[16]) goto CON4770;
	w=local[7];
	if (!iscons(w) && w!=NIL) error(E_NOLIST);
	w=(w)->c.cons.cdr;
	if (!iscons(w) && w!=NIL) error(E_NOLIST);
...
CON4770:
	...
	if (!iscons(w) && w!=NIL) error(E_NOLIST);
	local[16]= (w)->c.cons.car;
	if (fqv[39]!=local[16]) goto CON4771;
	w = fqv[39];
	ctx->vsp=local+16;
	local[0]=w;
	goto BLK4747;
...
/*make-lines*/
static pointer F4534make_lines(ctx,n,argv,env)
register context *ctx;
register int n; register pointer argv[]; pointer env;
{ register pointer *local=ctx->vsp, w, *fqv=qv;
  numunion nu;
	if (n!=1) maerror();
	ctx->vsp=local+0;
	local[0]= makeclosure(codevec,quotevec,CLO4774,env,argv,local);
	local[1]= argv[0];
	ctx->vsp=local+2;
...
/*closure or cleaner*/
static pointer CLO4774(ctx,n,argv,env)
register context *ctx;
register int n; register pointer argv[]; pointer env;
{ register pointer *local=ctx->vsp, w, *fqv=qv;
  numunion nu;
...
/*copy-face*/
static pointer F4535copy_face(ctx,n,argv,env)
register context *ctx;
register int n; register pointer argv[]; pointer env;
{ register pointer *local=ctx->vsp, w, *fqv=qv;
  numunion nu;
	if (n!=1) maerror();
	ctx->vsp=local+0;
	local[0]= makeclosure(codevec,quotevec,FLET4776,env,argv,local);
	...
	w = local[0];
	ctx->vsp=local+6;
	w=FLET4776(ctx,1,local+5,w);
	local[5]= w;
	local[6]= fqv[60];
	ctx->vsp=local+7;
	local[7]= makeclosure(codevec,quotevec,CLO4778,env,argv,local);
	local[8]= local[0];
	local[9]= argv[0];
	local[10]= fqv[60];
	ctx->vsp=local+11;
...
/*closure or cleaner*/
static pointer FLET4776(ctx,n,argv,env)
register context *ctx;
register int n; register pointer argv[]; pointer env;
{ register pointer *local=ctx->vsp, w, *fqv=qv;
  numunion nu;
...
/*closure or cleaner*/
static pointer CLO4778(ctx,n,argv,env)
register context *ctx;
register int n; register pointer argv[]; pointer env;
{ register pointer *local=ctx->vsp, w, *fqv=qv;
  numunion nu;
...
IF4786:
	...
	ctx->vsp=local+0;
	compfun(ctx,fqv[66],module,F4522coplanar_fe_intersection,fqv[67]);
	ctx->vsp=local+0;
	compfun(ctx,fqv[68],module,F4523find_next_segment,fqv[69]);
	ctx->vsp=local+0;
	compfun(ctx,fqv[70],module,F4524find_loop,fqv[71]);
	ctx->vsp=local+0;
	compfun(ctx,fqv[72],module,F4525punch_hole,fqv[73]);
	ctx->vsp=local+0;
	compfun(ctx,fqv[74],module,F4526construct_polygon,fqv[75]);
	ctx->vsp=local+0;
	compfun(ctx,fqv[76],module,F4527remove_non_overlapping_border,fqv[77]);
	ctx->vsp=local+0;
	compfun(ctx,fqv[78],module,F4528coplanar_ff_intersection,fqv[79]);
	ctx->vsp=local+0;
	compfun(ctx,fqv[80],module,F4529face_,fqv[81]);
	ctx->vsp=local+0;
	compfun(ctx,fqv[82],module,F4530face_,fqv[83]);
	ctx->vsp=local+0;
	compfun(ctx,fqv[84],module,F4531non_coplanar_fe_relation,fqv[85]);
	ctx->vsp=local+0;
	compfun(ctx,fqv[86],module,F4532ff_relation,fqv[87]);
	ctx->vsp=local+0;
	compfun(ctx,fqv[88],module,F4533bb_relation,fqv[89]);
	ctx->vsp=local+0;
	compfun(ctx,fqv[90],module,F4534make_lines,fqv[91]);
	ctx->vsp=local+0;
	compfun(ctx,fqv[92],module,F4535copy_face,fqv[93]);
	local[0]= fqv[94];
	local[1]= fqv[95];
	ctx->vsp=local+2;
