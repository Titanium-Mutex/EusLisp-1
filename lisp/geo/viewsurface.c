...
static pointer F3589hls2rgb();
static pointer F3590rgb2hls();
static pointer F3591tektro_showline();
static pointer F3592tektro_clear();
static pointer F3593default_viewsurface();

/*hls2rgb*/
static pointer F3589hls2rgb(ctx,n,argv,env)
register context *ctx;
register int n; register pointer argv[]; pointer env;
{ register pointer *local=ctx->vsp, w, *fqv=qv;
  numunion nu;
	if (n<3) maerror();
	if (n>=4) { local[0]=(argv[3]); goto ENT3596;}
	local[0]= makeint((eusinteger_t)255L);
ENT3596:
ENT3595:
	if (n>4) maerror();
	ctx->vsp=local+1;
	local[1]= makeclosure(codevec,quotevec,FLET3597,env,argv,local);
...
IF3599:
	...
	if (w==NIL) goto CON3601;
	if (argv[0]!=NIL) goto IF3602;
	local[4]= argv[1];
	local[5]= argv[1];
	local[6]= argv[1];
	ctx->vsp=local+7;
...
/*closure or cleaner*/
static pointer FLET3597(ctx,n,argv,env)
register context *ctx;
register int n; register pointer argv[]; pointer env;
{ register pointer *local=ctx->vsp, w, *fqv=qv;
  numunion nu;
...
IF3608:
	...
	local[1]= local[0];
	w = fqv[0];
	if (memq(local[1],w)==NIL) goto IF3610;
	local[1]= argv[0];
	local[2]= argv[1];
	local[3]= argv[0];
	ctx->vsp=local+4;
...
IF3610:
	local[1]= local[0];
	w = fqv[1];
	if (memq(local[1],w)==NIL) goto IF3612;
	local[1]= argv[1];
	goto IF3613;
IF3612:
	local[1]= local[0];
	w = fqv[2];
	if (memq(local[1],w)==NIL) goto IF3614;
	local[1]= argv[0];
	local[2]= argv[1];
	local[3]= argv[0];
	ctx->vsp=local+4;
...
IF3614:
	local[1]= local[0];
	w = fqv[3];
	if (memq(local[1],w)==NIL) goto IF3616;
	local[1]= argv[0];
	goto IF3617;
IF3616:
	local[1]= NIL;
...
/*rgb2hls*/
static pointer F3590rgb2hls(ctx,n,argv,env)
register context *ctx;
register int n; register pointer argv[]; pointer env;
{ register pointer *local=ctx->vsp, w, *fqv=qv;
  numunion nu;
	if (n<3) maerror();
	if (n>=4) { local[0]=(argv[3]); goto ENT3620;}
...
IF3626:
	...
	local[8] = w;
	local[10]= argv[0];
	if (local[4]!=local[10]) goto CON3628;
	local[10]= argv[1];
	if (local[5]!=local[10]) goto IF3629;
...
CON3628:
	local[10]= argv[1];
	if (local[4]!=local[10]) goto CON3631;
	local[10]= argv[2];
	if (local[5]!=local[10]) goto IF3632;
	local[10]= makeflt(1.0000000000000000000000e+00);
	{ double x,y;
...
CON3631:
	local[10]= argv[0];
	if (local[5]!=local[10]) goto IF3635;
	local[10]= makeflt(3.0000000000000000000000e+00);
	{ double x,y;
		y=fltval(local[7]); x=fltval(local[10]);
		local[10]=(makeflt(x + y));}
...
/*:drawline-primitive*/
static pointer M3637viewsurface_drawline_primitive(ctx,n,argv,env)
register context *ctx;
register int n; register pointer argv[]; pointer env;
{ register pointer *local=ctx->vsp, w, *fqv=qv;
  numunion nu;
	if (n<6) maerror();
	if (n>=7) { local[0]=(argv[6]); goto ENT3640;}
	local[0]= NIL;
ENT3640:
ENT3639:
	if (n>7) maerror();
...
/*:draw-line*/
static pointer M3641viewsurface_draw_line(ctx,n,argv,env)
register context *ctx;
register int n; register pointer argv[]; pointer env;
{ register pointer *local=ctx->vsp, w, *fqv=qv;
  numunion nu;
	if (n<4) maerror();
	if (n>=5) { local[0]=(argv[4]); goto ENT3644;}
	local[0]= NIL;
ENT3644:
ENT3643:
	if (n>5) maerror();
...
/*:line-style*/
static pointer M3645viewsurface_line_style(ctx,n,argv,env)
register context *ctx;
register int n; register pointer argv[]; pointer env;
{ register pointer *local=ctx->vsp, w, *fqv=qv;
  numunion nu;
	...
	local[0]= argv[2];
	local[1]= local[0];
	if (fqv[6]!=local[1]) goto IF3648;
	local[1]= fqv[7];
	goto IF3649;
IF3648:
	local[1]= local[0];
	if (fqv[8]!=local[1]) goto IF3650;
	local[1]= fqv[9];
	goto IF3651;
IF3650:
	local[1]= NIL;
...
/*:line-width*/
static pointer M3652viewsurface_line_width(ctx,n,argv,env)
register context *ctx;
register int n; register pointer argv[]; pointer env;
{ register pointer *local=ctx->vsp, w, *fqv=qv;
  numunion nu;
...
/*:nomethod*/
static pointer M3654viewsurface_nomethod(ctx,n,argv,env)
register context *ctx;
register int n; register pointer argv[]; pointer env;
{ register pointer *local=ctx->vsp, w, *fqv=qv;
  numunion nu;
...
/*:set-erase-mode*/
static pointer M3657viewsurface_set_erase_mode(ctx,n,argv,env)
register context *ctx;
register int n; register pointer argv[]; pointer env;
{ register pointer *local=ctx->vsp, w, *fqv=qv;
  numunion nu;
...
/*:set-show-mode*/
static pointer M3659viewsurface_set_show_mode(ctx,n,argv,env)
register context *ctx;
register int n; register pointer argv[]; pointer env;
{ register pointer *local=ctx->vsp, w, *fqv=qv;
  numunion nu;
...
/*:flush*/
static pointer M3661viewsurface_flush(ctx,n,argv,env)
register context *ctx;
register int n; register pointer argv[]; pointer env;
{ register pointer *local=ctx->vsp, w, *fqv=qv;
  numunion nu;
...
/*:init*/
static pointer M3663viewsurface_init(ctx,n,argv,env)
register context *ctx;
register int n; register pointer argv[]; pointer env;
{ register pointer *local=ctx->vsp, w, *fqv=qv;
  numunion nu;
...
/*:init*/
static pointer M3666tektro_viewsurface_init(ctx,n,argv,env)
register context *ctx;
register int n; register pointer argv[]; pointer env;
{ register pointer *local=ctx->vsp, w, *fqv=qv;
  numunion nu;
...
RST3668:
	...
	if (n & (1<<0)) goto KEY3669;
	local[1] = makeint((eusinteger_t)0L);
KEY3669:
	if (n & (1<<1)) goto KEY3670;
	local[2] = makeint((eusinteger_t)768L);
KEY3670:
	if (n & (1<<2)) goto KEY3671;
...
/*:clear*/
static pointer M3672tektro_viewsurface_clear(ctx,n,argv,env)
register context *ctx;
register int n; register pointer argv[]; pointer env;
{ register pointer *local=ctx->vsp, w, *fqv=qv;
  numunion nu;
	if (n!=2) maerror();
	ctx->vsp=local+0;
	w=(pointer)F3592tektro_clear(ctx,0,local+0); /*tektro-clear*/
	local[0]= w;
BLK3673:
	ctx->vsp=local; return(local[0]);}

/*:width*/
static pointer M3674tektro_viewsurface_width(ctx,n,argv,env)
register context *ctx;
register int n; register pointer argv[]; pointer env;
{ register pointer *local=ctx->vsp, w, *fqv=qv;
  numunion nu;
	if (n<2) maerror();
	if (n>=3) { local[0]=(argv[2]); goto ENT3677;}
	local[0]= NIL;
ENT3677:
ENT3676:
	if (n>3) maerror();
	w = local[0];
	if (!isnum(w)) goto IF3678;
...
/*:height*/
static pointer M3680tektro_viewsurface_height(ctx,n,argv,env)
register context *ctx;
register int n; register pointer argv[]; pointer env;
{ register pointer *local=ctx->vsp, w, *fqv=qv;
  numunion nu;
	if (n<2) maerror();
	if (n>=3) { local[0]=(argv[2]); goto ENT3683;}
	local[0]= NIL;
ENT3683:
ENT3682:
	if (n>3) maerror();
	w = local[0];
	if (!isnum(w)) goto IF3684;
...
/*:resize*/
static pointer M3686tektro_viewsurface_resize(ctx,n,argv,env)
register context *ctx;
register int n; register pointer argv[]; pointer env;
{ register pointer *local=ctx->vsp, w, *fqv=qv;
  numunion nu;
...
/*:drawline-primitive*/
static pointer M3688tektro_viewsurface_drawline_primitive(ctx,n,argv,env)
register context *ctx;
register int n; register pointer argv[]; pointer env;
{ register pointer *local=ctx->vsp, w, *fqv=qv;
  numunion nu;
	if (n<6) maerror();
	if (n>=7) { local[0]=(argv[6]); goto ENT3691;}
	local[0]= NIL;
ENT3691:
ENT3690:
	if (n>7) maerror();
	...
	local[4]= w;
	ctx->vsp=local+5;
	w=(pointer)F3591tektro_showline(ctx,4,local+1); /*tektro-showline*/
	local[0]= w;
BLK3689:
	ctx->vsp=local; return(local[0]);}

/*tektro-showline*/
static pointer F3591tektro_showline(ctx,n,argv,env)
register context *ctx;
register int n; register pointer argv[]; pointer env;
{ register pointer *local=ctx->vsp, w, *fqv=qv;
  numunion nu;
	if (n!=4) maerror();
	if (loadglobal(fqv[16])==NIL) goto IF3693;
	local[0]= T;
	local[1]= fqv[17];
	local[2]= argv[0];
	local[3]= argv[1];
...
/*tektro-clear*/
static pointer F3592tektro_clear(ctx,n,argv,env)
register context *ctx;
register int n; register pointer argv[]; pointer env;
{ register pointer *local=ctx->vsp, w, *fqv=qv;
  numunion nu;
	if (n<0) maerror();
	if (n>=1) { local[0]=(argv[0]); goto ENT3697;}
...
/*tektro*/
static pointer F3698(ctx,n,argv,env)
register context *ctx;
register int n; register pointer argv[]; pointer env;
{ register pointer *local=ctx->vsp, w, *fqv=qv;
  numunion nu;
...
/*default-viewsurface*/
static pointer F3593default_viewsurface(ctx,n,argv,env)
register context *ctx;
register int n; register pointer argv[]; pointer env;
{ register pointer *local=ctx->vsp, w, *fqv=qv;
  numunion nu;
...
IF3704:
	...
	ctx->vsp=local+0;
	compfun(ctx,fqv[31],module,F3589hls2rgb,fqv[32]);
	ctx->vsp=local+0;
	compfun(ctx,fqv[33],module,F3590rgb2hls,fqv[34]);
	local[0]= fqv[35];
	local[1]= fqv[36];
	local[2]= fqv[35];
	local[3]= fqv[37];
	...
	w=(pointer)SEND(ctx,3,local+0); /*send*/
	ctx->vsp=local+0;
	addcmethod(ctx,module,M3637viewsurface_drawline_primitive,fqv[5],fqv[35],fqv[46]);
	ctx->vsp=local+0;
	addcmethod(ctx,module,M3641viewsurface_draw_line,fqv[47],fqv[35],fqv[48]);
	ctx->vsp=local+0;
	addcmethod(ctx,module,M3645viewsurface_line_style,fqv[49],fqv[35],fqv[50]);
	ctx->vsp=local+0;
	addcmethod(ctx,module,M3652viewsurface_line_width,fqv[51],fqv[35],fqv[52]);
	ctx->vsp=local+0;
	addcmethod(ctx,module,M3654viewsurface_nomethod,fqv[53],fqv[35],fqv[54]);
	ctx->vsp=local+0;
	addcmethod(ctx,module,M3657viewsurface_set_erase_mode,fqv[55],fqv[35],fqv[56]);
	ctx->vsp=local+0;
	addcmethod(ctx,module,M3659viewsurface_set_show_mode,fqv[57],fqv[35],fqv[58]);
	ctx->vsp=local+0;
	addcmethod(ctx,module,M3661viewsurface_flush,fqv[59],fqv[35],fqv[60]);
	ctx->vsp=local+0;
	addcmethod(ctx,module,M3663viewsurface_init,fqv[25],fqv[35],fqv[61]);
	local[0]= fqv[24];
	local[1]= fqv[36];
	local[2]= fqv[24];
	local[3]= fqv[37];
	...
	w=(pointer)SEND(ctx,3,local+0); /*send*/
	ctx->vsp=local+0;
	addcmethod(ctx,module,M3666tektro_viewsurface_init,fqv[25],fqv[24],fqv[63]);
	ctx->vsp=local+0;
	addcmethod(ctx,module,M3672tektro_viewsurface_clear,fqv[64],fqv[24],fqv[65]);
	ctx->vsp=local+0;
	addcmethod(ctx,module,M3674tektro_viewsurface_width,fqv[14],fqv[24],fqv[66]);
	ctx->vsp=local+0;
	addcmethod(ctx,module,M3680tektro_viewsurface_height,fqv[15],fqv[24],fqv[67]);
	ctx->vsp=local+0;
	addcmethod(ctx,module,M3686tektro_viewsurface_resize,fqv[68],fqv[24],fqv[69]);
	ctx->vsp=local+0;
	addcmethod(ctx,module,M3688tektro_viewsurface_drawline_primitive,fqv[5],fqv[24],fqv[70]);
	local[0]= fqv[18];
	local[1]= fqv[71];
	ctx->vsp=local+2;
...
IF3710:
	ctx->vsp=local+0;
	compfun(ctx,fqv[73],module,F3591tektro_showline,fqv[74]);
	ctx->vsp=local+0;
	compfun(ctx,fqv[75],module,F3592tektro_clear,fqv[76]);
	ctx->vsp=local+0;
	compmacro(ctx,fqv[77],module,F3698,fqv[78]);
	ctx->vsp=local+0;
	compfun(ctx,fqv[79],module,F3593default_viewsurface,fqv[80]);
	local[0]= fqv[81];
	local[1]= fqv[82];
	ctx->vsp=local+2;
